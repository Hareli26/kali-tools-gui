...
- **📚 חומרי למידה** — מה המערכת למדה + Q&A חינוכי לכל סוג איום (מה זה, איך מתגוננים, איך מזהים, תיקון, MITRE).
  כולל קטע **🍯 מה נלמד מהמלכודות** — טכניקות שתוקפים אמיתיים ניסו בפועל (ATTACK_KB), עם מונה, ומודגשות כשהן **מוצלבות** לחולשה קיימת.
...
- **Frontend** — `web/`: SPA ב‑vanilla JS (ללא build), RTL, ערכת נושא כהה, ~12 מסכים (כלים, AI,
  בקרה, היסטוריה, למידה, Obsidian, 🍯 מלכודות, משתמשים, Playbooks ועוד). זרימת מצב הכלים — 3 מסכים:
...
- **קטלוג** — `tools.json`: 74 כלים ב‑11 קטגוריות (Recon, DNS, Network, Web, SMB,
